...
/**
 * Servlet implementation class RequestStoreListAjaxPagingServlet
 */
@WebServlet("/admin/store/ajaxPaging")
public class RequestStoreListAjaxPagingServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RequestStoreListAjaxPagingServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List list = new AdminService().selectRequestStore(cPage,numPerPage);
		if(list.size()>0) {
			int totalMember =  new AdminService().requestStoreCount();
